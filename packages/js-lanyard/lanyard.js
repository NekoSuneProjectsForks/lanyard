...
async function lanyard(opts) {
    ...
    // Point apiUrl/websocketUrl at a self-hosted Lanyard instance if you run one.
    const API_URL = opts.apiUrl || CONSTANTS.API_URL;
    const WEBSOCKET_URL = opts.websocketUrl || CONSTANTS.WEBSOCKET_URL;
    ...
    if (opts.socket) {
        ...
        if (typeof opts.userId == "string") {
            const res = await fetch(`${API_URL}/users/${opts.userId}`);
