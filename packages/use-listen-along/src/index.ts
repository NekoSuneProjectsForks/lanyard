 ...
 * @param apiUrl Base URL of the Lanyard instance to poll. Defaults to the public API.
 */
export function useListenAlong(
  snowflake: string,
  auth: string,
  disconnect?: boolean,
  apiUrl: string = 'https://api.lanyard.rest',
) {
  const lanyardApiUrl = apiUrl.replace(/\/$/, '');
  ...
  setInterval(() => {
    if (!disconnect) {
      fetch(`${lanyardApiUrl}/v1/users/${snowflake}`)
        .then((r) => r.json())
        .then(parseResponse)
        .then(setResponse);
    }
